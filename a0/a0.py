...
def friend_overlap(users):
    ...
    for i in range(len(users)):
        for j in range(i + 1, len(users)):
            overlapNumber = len(set(users[i]['friends']) & set(users[j]['friends']))
            result = (users[i]['screen_name'], users[j]['screen_name'], overlapNumber)
            resultSet.append(result)
    return sorted(resultSet, key=lambda x: (-x[2], x[0], x[1]))
...
def followed_by_hillary_and_donald(users, twitter):
    ...
    """
    # friend_donald = set(get_friends(twitter, 'realDonaldTrump'))
    # print(friend_donald)
    # friend_hillary = set(get_friends(twitter, 'HillaryClinton'))
    # print(friend_hillary)
    for user in users:
        if user['screen_name'] == 'realDonaldTrump':
            friend_donald = set(user['friends'])
        elif user['screen_name'] == 'HillaryClinton':
            friend_hillary = set(user['friends'])
    friend_set_int = friend_donald & friend_hillary
    # print(friend_set_int)
    resultSet = []
    for i in friend_set_int:
        response = robust_request(twitter, "users/lookup", {'user_id': i}).json()
        for user in response:
            resultSet.append(user['screen_name'])
    return resultSet
...
if __name__ == '__main__':
    ...
    # check read_screen_names
    # print(read_screen_names('candidates.txt'))

    # twitter = get_twitter()

    # check get_users function
    # users = get_users(twitter, ['twitterapi', 'twitter'])
    # print([u['id'] for u in users])

    # check get_friends function
    # friends = get_friends(twitter, ['aronwc'])[:5]
    # print(friends)

    # check add_all_friends function
    # users = [{'screen_name': 'aronwc'}]
    # add_all_friends(twitter, users)
    # print(users[0]['friends'][:5])

    # check print_num_friends function
    # users = [{'screen_name': 'aronwc'}, {'screen_name': 'JH'}]
    # add_all_friends(twitter, users)
    # print_num_friends(users)

    # check count_friends function
    # c = count_friends([{'friends': [1, 2]}, {'friends': [2, 3]}, {'friends': [2, 3]}])
    # print(c.most_common())

    # check friend_overlap function
    # print(friend_overlap([
    #     {'screen_name': 'a', 'friends': ['1', '2', '3']},
    #     {'screen_name': 'b', 'friends': ['2', '3', '4']},
    #     {'screen_name': 'c', 'friends': ['1', '2', '3']},
    # ]))
